type Props = {
  ...
  setAssetOptions: React.Dispatch<React.SetStateAction<number[]>>;
  setFamilyOptions: React.Dispatch<React.SetStateAction<number[]>>;
  handleCreateDiagram: () => void;
  toggleAsset: (val: number) => void;
...
const SelectPage = ({
  ...
  return (
    <div className="flex space-x-6 p-4 border rounded-md max-w-lg w-full">
      ...
      <button className="bg-blue-500 text-white px-4 py-2 rounded self-start" onClick={handleCreateDiagram}>
        Create Diagram
      </button>
    </div>
  );
